...
test('Login through Auth0 should succeed with valid credentials', async ({ page }) => {
  ...
  // 2. Нажать кнопку входа через Auth0
  ...
  // 6. Ждём редирект обратно на localhost
  await page.waitForURL('http://localhost:3000/**', { timeout: 15000 });
  
  // 7. Проверяем успешный вход (наличие user menu или аватара)
  await expect(page.locator('[data-testid="user-menu"], .user-avatar, header [aria-label="User menu"]')).toBeVisible({ timeout: 10000 });
