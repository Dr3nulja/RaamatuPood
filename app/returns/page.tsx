...
export default function ReturnsPage() {
	return (
		<main className="min-h-screen bg-gradient-to-b from-[#FDF8F0] to-[#F5F0E8] px-4 py-12 text-zinc-900 md:px-6">
			<div className="mx-auto flex w-full max-w-6xl flex-col gap-8">
				<section className="rounded-3xl border border-amber-100 bg-white px-6 py-8 shadow-sm md:px-10 md:py-12">
					<p className="inline-flex rounded-full bg-amber-100 px-3 py-1 text-sm font-semibold text-[#8B5E3C]">
						Возвраты и обмен
					</p>
					<h1 className="mt-4 font-serif text-3xl font-bold text-[#8B5E3C] md:text-5xl">Простые и прозрачные правила</h1>
				...
				<section className="grid gap-4 md:grid-cols-3">
					{rules.map((rule) => (
						...
						>
							<h2 className="font-serif text-xl font-semibold text-[#8B5E3C]">{rule.title}</h2>
				...
				<section className="rounded-3xl border border-amber-100 bg-white p-6 shadow-sm md:p-8">
					<h2 className="font-serif text-2xl font-bold text-[#8B5E3C] md:text-3xl">Пошаговая инструкция</h2>
					<ol className="mt-6 grid gap-4 md:grid-cols-2">
						{steps.map((step, index) => (
							<li
								key={step}
								className="flex items-start gap-3 rounded-2xl border border-amber-100 bg-[#FDF8F0] p-4 transition hover:bg-amber-50"
							>
								<span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#A0785A] text-sm font-bold text-white">
									{index + 1}
								</span>
				...
				<section className="rounded-3xl border border-amber-100 bg-white p-6 shadow-sm md:p-8">
					<h2 className="font-serif text-2xl font-bold text-[#8B5E3C] md:text-3xl">Что не подлежит возврату</h2>
					<ul className="mt-4 space-y-3">
						{exceptions.map((item) => (
							<li key={item} className="rounded-xl bg-[#F5F0E8] px-4 py-3 text-sm text-zinc-700">
								{item}
							</li>
						))}
					</ul>
				...
				<section className="rounded-3xl border border-amber-200 bg-white p-6 shadow-sm md:p-8">
					<h2 className="font-serif text-2xl font-bold text-[#8B5E3C] md:text-3xl">Контакты для возврата</h2>
					...
					<Link
						href="/contacts"
						className="mt-5 inline-flex rounded-xl bg-[#D97706] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#F59E0B] focus:outline-none focus:ring-2 focus:ring-amber-300"
					>
						Написать нам
					</Link>
				</section>
				...
				<section className="rounded-2xl border border-amber-100 bg-white px-5 py-4 text-sm text-zinc-700 shadow-sm">
					...
					<Link
						href="/#"
						className="ml-1 font-semibold text-[#8B5E3C] underline-offset-4 transition hover:text-[#A0785A] hover:underline focus:outline-none focus:ring-2 focus:ring-amber-300"
					>
						FAQ
					</Link>
					или перейдите в
