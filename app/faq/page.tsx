...
export default function FaqPage() {
	return (
		<main className="min-h-screen bg-gradient-to-b from-[#FDF8F0] to-[#F5F0E8] px-4 py-12 md:px-6">
			<div className="mx-auto w-full max-w-5xl">
				<section className="rounded-3xl border border-amber-100 bg-white px-6 py-8 shadow-sm md:px-10 md:py-12">
					<h1 className="font-serif text-3xl font-bold text-[#8B5E3C] md:text-5xl">Часто задаваемые вопросы</h1>
				...
				<section className="mt-8 space-y-3">
					{faqItems.map((item) => (
						...
						>
							<summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-serif text-lg font-semibold text-[#8B5E3C] outline-none focus-visible:ring-2 focus-visible:ring-amber-300 rounded-lg">
								<span>{item.question}</span>
								<span className="text-[#A0785A] transition-transform duration-300 group-open:rotate-45">+</span>
							</summary>
				...
				<section className="mt-8 rounded-3xl border border-amber-200 bg-white px-6 py-7 shadow-sm md:px-8 md:py-9">
					<h2 className="font-serif text-2xl font-bold text-[#8B5E3C] md:text-3xl">Всё ещё есть вопросы?</h2>
					<p className="mt-2 text-zinc-700">Наша команда с радостью поможет и подскажет лучший вариант решения.</p>
					<Link
						href="/contacts"
						className="mt-5 inline-flex rounded-xl bg-[#D97706] px-5 py-3 text-sm font-semibold text-white transition-all duration-200 hover:bg-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-300"
					>
						Написать нам
					</Link>
				</section>
